...
static int trx_usrp_write(openair0_device *device,
  ...
  #ifdef write_usrp
  char filename[100];
  sprintf(filename, "/tmp/usrp/write/write%lld.pcm", timestamp);
...
VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_BEAM_SWITCHING_GPIO,0);
  ...
  // LOG_I(HW, "cc %d flags %d nsamps %d nsamps2 %d s->tx_count %d\n", cc, flags, nsamps, nsamps2, s->tx_count);
...
static int trx_usrp_read(openair0_device *device, openair0_timestamp *ptimestamp, void **buff, int nsamps, int cc) {
  ...
  *ptimestamp = s->rx_timestamp;
    ...
    #ifdef read_usrp
        char filename[100];
      sprintf(filename, "/tmp/usrp/read/read%lld.pcm", *ptimestamp);
      FILE *file = fopen(filename, "a+");
      fwrite(&((int16_t *)buff[0])[0], nsamps*sizeof(int), 1, file);
      fclose(file);
    #endif
  ...
  if (device->type==USRP_N300_DEV || device->type==USRP_X300_DEV) {
    ...
    switch ((int)openair0_cfg[0].sample_rate) {
      ...
      case 30720000:
        ...
        //openair0_cfg[0].samples_per_packet    = 2048;
        openair0_cfg[0].tx_sample_advance     = 15;
        openair0_cfg[0].tx_bw                 = 100e6; // 20e6
        openair0_cfg[0].rx_bw                 = 100e6; // 20e6
        break;
